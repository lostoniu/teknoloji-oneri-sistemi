import osimport jsonimport randomimport pandas as pdimport streamlit as stimport psycopg2

from recommender import (urun_oner,chatbot_metnini_anla,aciklama_uret,)

from anything_client import llm_analiz_et, llm_sohbet_et

from auth import (register_user,verify_user,login_user,sifre_sifirlama_kodu_gonder,sifreyi_guncelle)
...
def get_secret_value(key, default=None):env_value = os.getenv(key)if env_value not in [None, ""]:return env_value
...
def get_db_connection():return psycopg2.connect(host=get_secret_value("DB_HOST"),database=get_secret_value("DB_NAME"),user=get_secret_value("DB_USER"),password=get_secret_value("DB_PASSWORD"),port=get_secret_value("DB_PORT", "5432"))

def safe_json_dumps(data):try:return json.dumps(data, ensure_ascii=False, default=str)except Exception:return "{}"

def favori_ekle(user_id, product_type, product_name, product_data):try:conn = get_db_connection()cur = conn.cursor()
...
def favorileri_getir(user_id):try:conn = get_db_connection()cur = conn.cursor()
...
def sistemi_kaydet(user_id, build_name, build_data, total_price):try:conn = get_db_connection()cur = conn.cursor()
...
def kayitli_sistemleri_getir(user_id):try:conn = get_db_connection()cur = conn.cursor()
...
if "pc_build" not in st.session_state:st.session_state.pc_build = None

if "pc_builds" not in st.session_state:st.session_state.pc_builds = []

if "selected_pc_build_index" not in st.session_state:st.session_state.selected_pc_build_index = 0

if "aktif_mod" not in st.session_state:st.session_state.aktif_mod = None

if "kategori" not in st.session_state:st.session_state.kategori = "Telefon"

if "min_butce" not in st.session_state:st.session_state.min_butce = 0

if "max_butce" not in st.session_state:st.session_state.max_butce = 30000

if "min_ram" not in st.session_state:st.session_state.min_ram = 0

if "mesajlar" not in st.session_state:st.session_state.mesajlar = []

if "pc_random_seed" not in st.session_state:st.session_state.pc_random_seed = 1

def _json_data_to_df(rows):temiz_rows = []
...
@st.cache_data(ttl=86400, show_spinner=False)def products_db_yukle(source):try:with get_db_connection() as conn:df = pd.read_sql("""SELECT source, kategori, alt_kategori, marka, model, fiyat_tl, puan, populerlik, dataFROM productsWHERE source = %s""",conn,params=[source])
...
@st.cache_data(ttl=86400, show_spinner=False)def ev_esyalari_yukle():df = products_db_yukle("ev")
...
@st.cache_data(ttl=86400, show_spinner=False)def pc_dataset_yukle():df = products_db_yukle("pc")
...
@st.cache_data(ttl=86400, show_spinner=False)def teknoloji_dataset_yukle():df = products_db_yukle("teknoloji")
...
ev_df = ev_esyalari_yukle()pc_df = pc_dataset_yukle()tech_df = teknoloji_dataset_yukle()



def kart_puani_getir(row):for kolon in ["Topluluk_Puani", "TOPLULUK_PUANI", "ONERI_PUANI", "Puan"]:try:if kolon in row.index and str(row[kolon]) != "nan":puan = float(row[kolon])
...
def kart_populerlik_getir(row):if "Populerlik" in row.index and str(row["Populerlik"]) != "nan":return row["Populerlik"]
...
def fiyat_formatla(deger):try:return f"{int(float(deger)):,}".replace(",", ".") + " TL"except Exception:return str(deger)

def veri_getir(row, kolon):if kolon in row.index and str(row[kolon]) != "nan":return row[kolon]return "Yok"

def sayisal_filtre_degeri(seri):return seri.astype(str).str.extract(r"(\d+)")[0].fillna(0).astype(int)

def sayi_cek(deger):try:if pd.isna(deger):return 0
...
def ev_ana_kategorileri_getir():if ev_df.empty:return ["Dataset bulunamadı"]

return sorted(ev_df["Ana_Kategori"].dropna().unique())

def ev_alt_kategorileri_getir():if ev_df.empty:return ["Dataset bulunamadı"]

return sorted(ev_df["Alt_Kategori"].dropna().unique())

def marka_listesi_getir(df, kategori_kolon=None, kategori_deger=None, ana_kolon=None, ana_deger=None):"""Verilen dataframe içinden güvenli şekilde marka listesi üretir."""if df is None or df.empty or "Marka" not in df.columns:return ["Farketmez"]
...
def temizle_yazi(x):x = str(x).strip().lower()x = x.replace("ı", "i")x = x.replace("İ", "i")x = x.replace("i̇", "i")x = x.replace("ğ", "g")x = x.replace("ü", "u")x = x.replace("ş", "s")x = x.replace("ö", "o")x = x.replace("ç", "c")return x

def hazir_urun_oner_db(kategori, min_butce, max_butce, min_ram, siralama, kullanim=""):if tech_df.empty:return pd.DataFrame()
...
def ev_alt_kategori_tipi(alt_kategori):"""Elektronik ev eşyasında sadece ilgili filtreler çıksın diye alt kategori tipini belirler."""a = temizle_yazi(str(alt_kategori))
...
GUVENILIR_SITE_LINKLERI = {"Amazon Türkiye": "https://www.amazon.com.tr/s?k={q}","Amazon": "https://www.amazon.com.tr/s?k={q}","Hepsiburada": "https://www.hepsiburada.com/ara?q={q}","Trendyol": "https://www.trendyol.com/sr?q={q}","Teknosa": "https://www.teknosa.com/arama/?s={q}","MediaMarkt": "https://www.mediamarkt.com.tr/tr/search.html?query={q}","Vatan Bilgisayar": "https://www.vatanbilgisayar.com/arama/{q}/","Vatan": "https://www.vatanbilgisayar.com/arama/{q}/","n11": "https://www.n11.com/arama?q={q}","İtopya": "https://www.itopya.com/arama/?search={q}","Itopya": "https://www.itopya.com/arama/?search={q}","İncehesap": "https://www.incehesap.com/arama/?q={q}","Incehesap": "https://www.incehesap.com/arama/?q={q}","Sinerji": "https://www.sinerji.gen.tr/arama?q={q}","GamingGenTR": "https://www.gaming.gen.tr/?s={q}&post_type=product","Akakçe": "https://www.akakce.com/arama/?q={q}","Akakce": "https://www.akakce.com/arama/?q={q}","Epey": "https://www.epey.com/arama/?q={q}",}

GUVENILIR_SITE_ADLARI = list(GUVENILIR_SITE_LINKLERI.keys())

def html_escape(deger):return (str(deger).replace("&", "&").replace("<", "<").replace(">", ">").replace('"', """))

def site_guvenilir_mi(site_adi):site = str(site_adi).strip().lower()if site in ["", "nan", "none", "yok"]:return Falsefor guvenilir in GUVENILIR_SITE_ADLARI:guvenilir_lower = guvenilir.lower()if site == guvenilir_lower or guvenilir_lower in site or site in guvenilir_lower:return Truereturn False

def site_adi_duzelt(site_adi):site = str(site_adi).strip()for guvenilir in GUVENILIR_SITE_ADLARI:guvenilir_lower = guvenilir.lower()site_lower = site.lower()if site_lower == guvenilir_lower or guvenilir_lower in site_lower or site_lower in guvenilir_lower:return guvenilirreturn site

def guvenilir_site_arama_linki(site_adi, urun_adi):from urllib.parse import quote_plussite = site_adi_duzelt(site_adi)q = quote_plus(str(urun_adi))kalip = GUVENILIR_SITE_LINKLERI.get(site)if kalip:return kalip.format(q=q)return "https://www.google.com/search?q=" + q

def urun_adi_olustur(row):marka = veri_getir(row, "Marka")model = veri_getir(row, "Model")if marka != "Yok" and model != "Yok":return f"{marka} {model}"if model != "Yok":return str(model)return str(row.get("Model", "Ürün"))

def ayni_urun_kaynaklari(kaynak_turu, row):if kaynak_turu == "ev":df = ev_df.copy()site_kolon = "Kaynak_Site"fiyat_kolon = "Fiyat_TL"marka = str(veri_getir(row, "Marka"))model = str(veri_getir(row, "Model"))if df.empty or site_kolon not in df.columns:return pd.DataFrame()adaylar = df[(df["Marka"].astype(str) == marka) & (df["Model"].astype(str) == model)].copy()elif kaynak_turu == "pc":df = pc_df.copy()site_kolon = "Kaynak"fiyat_kolon = "Fiyat_TL"marka = str(veri_getir(row, "Marka"))model = str(veri_getir(row, "Model"))if df.empty or site_kolon not in df.columns:return pd.DataFrame()adaylar = df[(df["Marka"].astype(str) == marka) & (df["Model"].astype(str) == model)].copy()else:return pd.DataFrame()if adaylar.empty:return adaylaradaylar = adaylar[adaylar[site_kolon].apply(site_guvenilir_mi)].copy()if adaylar.empty:return adaylaradaylar[fiyat_kolon] = pd.to_numeric(adaylar[fiyat_kolon], errors="coerce").fillna(0).astype(int)adaylar = adaylar[adaylar[fiyat_kolon] > 0]adaylar = adaylar.sort_values(by=fiyat_kolon, ascending=True)adaylar = adaylar.drop_duplicates(subset=[site_kolon], keep="first")adaylar = adaylar.rename(columns={site_kolon: "Site", fiyat_kolon: "Fiyat"})return adaylar[["Site", "Fiyat"]].head(5)

def fiyat_karsilastirma_html(kaynak_turu, row):urun_adi = urun_adi_olustur(row)return f"""

def hazir_urun_guvenilir_link_html(row):model = str(veri_getir(row, "Model"))return f"""

MARKA_KELIMELERI = ["Apple", "Samsung", "Xiaomi", "Huawei", "Honor", "OnePlus", "Oppo", "Vivo", "Realme","Lenovo", "HP", "Dell", "Asus", "Acer", "MSI", "Monster", "Casper", "Huawei","Sony", "JBL", "Anker", "Sennheiser", "Beats", "Logitech", "Philips", "TCL","Garmin", "Amazfit", "Fitbit"]

def modelden_marka_bul(model):metin = str(model).lower()
...
def hazir_urun_markalari_getir(kategori):if tech_df.empty or "Kategori" not in tech_df.columns or "Model" not in tech_df.columns:return ["Farketmez"]
...
def secenekleri_getir(df, kategori, kolon):if df.empty or "Kategori" not in df.columns or kolon not in df.columns:return ["Farketmez"]
...
def kapasite_gb_cek(deger):metin = str(deger).lower().replace(",", ".")
...
def evet_hayir_filtrele(df, kolon, secim):if secim == "Farketmez" or kolon not in df.columns:return df
...
def ekran_hz_cek(deger):import remetin = str(deger).lower()hzler = re.findall(r"(\d+)\s*hz", metin)if hzler:return max([int(x) for x in hzler])sayilar = re.findall(r"\d+", metin)if sayilar:adaylar = [int(x) for x in sayilar if int(x) in [60, 75, 90, 120, 144, 165, 240, 300, 360]]if adaylar:return max(adaylar)return 0

def cpu_seviye_uygun_mu(islemci, seviye):if seviye == "Farketmez":return Truei = str(islemci).lower()if seviye == "Giriş":return any(k in i for k in ["i3", "ryzen 3", "celeron", "pentium"])if seviye == "Orta":return any(k in i for k in ["i5", "ryzen 5", "m1", "m2"])if seviye == "Üst":return any(k in i for k in ["i7", "ryzen 7", "m3"])if seviye == "Premium":return any(k in i for k in ["i9", "ryzen 9", "m4", "ultra 9"])return True

def gpu_seviye_uygun_mu(gpu, seviye):if seviye == "Farketmez":return Trueg = str(gpu).lower()if seviye == "Paylaşımlı":return any(k in g for k in ["uhd", "iris", "integrated", "paylaşımlı", "dahili"])if seviye == "Giriş":return any(k in g for k in ["rtx 3050", "rtx 4050", "gtx", "mx"])if seviye == "Orta":return any(k in g for k in ["rtx 3060", "rtx 4060", "rtx 3070", "rtx 4070", "rx 7600", "rx 7700"])if seviye == "Üst":return any(k in g for k in ["rtx 4080", "rtx 4090", "rtx 5080", "rtx 5090", "rx 7900"])return True

def hazir_urun_detay_filtrele(df, kategori, filtreler):if df is None or df.empty:return df
...
def karsilastirma_urun_adi(row):marka = veri_getir(row, "Marka")model = veri_getir(row, "Model")
...
def karsilastirma_fiyat(row):for kolon in ["Fiyat_TL", "FIYAT_SAYI", "Fiyat (TL)"]:if kolon in row.index and str(row[kolon]) != "nan":return fiyat_formatla(row[kolon])return "Yok"

def karsilastirma_puan(row):if "kart_puani_getir" in globals():return kart_puani_getir(row)
...
def karsilastirma_satiri_uret(ozellik, row1, row2, kolonlar):deger1 = "Yok"deger2 = "Yok"
...
def karsilastirma_tablo_olustur(row1, row2, kaynak_turu):satirlar = [{"Özellik": "Ürün", "1. Ürün": karsilastirma_urun_adi(row1), "2. Ürün": karsilastirma_urun_adi(row2)},{"Özellik": "Ortalama Piyasa Fiyatı", "1. Ürün": karsilastirma_fiyat(row1), "2. Ürün": karsilastirma_fiyat(row2)},{"Özellik": "Puan", "1. Ürün": karsilastirma_puan(row1), "2. Ürün": karsilastirma_puan(row2)},karsilastirma_satiri_uret("Marka", row1, row2, ["Marka"]),karsilastirma_satiri_uret("Kategori", row1, row2, ["Kategori", "Ana_Kategori"]),karsilastirma_satiri_uret("Alt Kategori", row1, row2, ["Alt_Kategori"]),karsilastirma_satiri_uret("Segment", row1, row2, ["Segment"]),]
...
def karsilastirma_dataframe_getir(kaynak_turu, kategori_secimi):if kaynak_turu == "Hazır Teknoloji Ürünleri":if tech_df.empty:return pd.DataFrame()
...
def karsilastirma_urun_listesi(df):if df.empty:return []
...
def siralama_uygula(df, siralama, fiyat_kolon=None, puan_kolon=None, yorum_kolon=None, ram_kolon=None):if df is None or df.empty:return df
...
def pc_parcalari_getir(alt_kategori, min_butce=0, max_butce=250000):if pc_df.empty:return pd.DataFrame()
...
def pc_parca_filtrele(alt_kategori,min_butce,max_butce,soket,ram_tipi,min_vram,min_kapasite,min_watt,rgb,siralama,marka="Farketmez"):sonuc = pc_parcalari_getir(alt_kategori, min_butce, max_butce)
...
def pc_random_sec(df, seed):if df is None or df.empty:return None
...
def pc_aday_sec(adaylar, hedef_fiyat, seed):if adaylar is None or adaylar.empty:return None
...
def fiyat_hedefine_yakin_satir(df, hedef, varyasyon=0):if df is None or df.empty:return None
...
def pc_havuzu_fast(alt_kategori, max_butce, kullanim=""):if pc_df.empty:return pd.DataFrame()
...
def uyumlu_havuz_filtrele(df, kategori, soket=None, ram_tipi=None, min_psu=0):if df is None or df.empty:return pd.DataFrame()
...
def sistem_toplam_fiyat(parcalar):return sum(int(row.get("Fiyat_TL", 0)) for row in parcalar.values())

def sistem_imza(parcalar):return "|".join([str(row.get("Marka", "")) + " " + str(row.get("Model", ""))for row in parcalar.values()])

def sistemi_butceye_yaklastir(parcalar, havuzlar, min_butce, max_butce, hedef_fiyat):# En fazla 10 ucuz/pahalı değişim yapar. Sonsuz döngü yok, hızlı çalışır.for _ in range(10):toplam = sistem_toplam_fiyat(parcalar)
...
def pc_kurulum_kategorileri(kurulum_tipi="Temel Sistem"):temel = ["İşlemci","Ekran Kartı","Anakart","RAM","SSD","Güç Kaynağı","Kasa","Soğutucu"]
...
def pc_butce_oranlari(kurulum_tipi="Temel Sistem"):if kurulum_tipi == "Tam Kurulum":return {"İşlemci": 0.15,"Ekran Kartı": 0.30,"Anakart": 0.09,"RAM": 0.08,"SSD": 0.07,"HDD": 0.04,"Güç Kaynağı": 0.06,"Kasa": 0.05,"Soğutucu": 0.04,"Monitör": 0.09,"Klavye": 0.015,"Mouse": 0.015,}
...
def tek_sistem_fast_olustur(min_butce, max_butce, hedef_fiyat, kullanim, seed, ana_havuzlar, varyasyon, kurulum_tipi="Temel Sistem"):oranlar = pc_butce_oranlari(kurulum_tipi)parcalar = {}
...
def besli_pc_sistem_olustur(min_butce, max_butce, kullanim, seed, kurulum_tipi="Temel Sistem"):if pc_df.empty:return []
...
def uyumlu_pc_sistem_topla(max_butce, kullanim, seed):sistemler = besli_pc_sistem_olustur(0, max_butce, kullanim, seed, kurulum_tipi="Temel Sistem")if sistemler:return sistemler[0]return {"basarili": False,"mesaj": "Bu bütçeye uygun sistem oluşturulamadı.","parcalar": {},"toplam_fiyat": 0,"butce": max_butce}

def ev_esyasi_oner(ana_kategori,alt_kategori,min_butce,max_butce,siralama,kullanim,min_puan,enerji_sinifi,kaynak_site,min_watt="Farketmez",min_emis="Farketmez",min_hazne="Farketmez",wifi="Farketmez",rgb="Farketmez",min_garanti="Farketmez",stok="Farketmez",marka="Farketmez"):if ev_df.empty:return pd.DataFrame()
...
def auth_screen():st.title("🤖 Akıllı Teknoloji Ürünleri Öneri Sistemi")st.write("Uygulamayı kullanmak için giriş yapmalı veya kayıt olmalısın.")
...
def pc_parca_karti(row):st.markdown(f"""

def ev_karti(row):st.markdown(f"""

st.sidebar.markdown("### 🧪 Test Modu")

if st.sidebar.button("Test Kullanıcısı Olarak Gir", key="test_login_button"):st.session_state.logged_in = Truest.session_state.user = {"id": 0,"username": "test_user","email": "test@example.com"}st.rerun()

if not st.session_state.logged_in:auth_screen()st.stop()



def chatbot_kategori_bul(mesaj_lower):kategori_kelimeleri = {"Toplama Bilgisayar": ["toplama", "sistem topla", "pc topla", "oyun sistemi", "parça", "parca"],"Elektronik Ev Eşyaları": ["süpürge", "supurge", "airfryer", "kahve", "klima", "ütü", "utu", "kettle", "blender", "ev eşyası", "ev esyasi", "çay makinesi", "cay makinesi", "robot", "dikey süpürge", "hava temizleyici", "vantilatör", "kombi", "kedi tuvaleti"],"Bilgisayar": ["laptop", "notebook", "bilgisayar", "oyun bilgisayarı", "oyun bilgisayari"],"Telefon": ["telefon", "iphone", "android", "samsung", "xiaomi", "redmi", "oppo", "vivo", "realme"],"Tablet": ["tablet", "ipad"],"Kulaklık": ["kulaklık", "kulaklik", "earbuds", "airpods", "kulak üstü", "kulak ustu"],"Akıllı Saat / Bileklik": ["akıllı saat", "akilli saat", "bileklik", "watch", "akıllı bileklik", "akilli bileklik"],}for kategori_adi, kelimeler in kategori_kelimeleri.items():if any(kelime in mesaj_lower for kelime in kelimeler):return kategori_adireturn None



def chatbot_ev_alt_kategori_bul(mesaj_lower):"""Chatbotun elektronik ev eşyasında alt kategoriyi paneldeki filtreler kadar doğru yakalaması için kullanılır."""alt_map = {"Dikey Süpürge": ["dikey süpürge", "dikey supurge", "şarjlı süpürge", "sarjli supurge"],"Robot Süpürge": ["robot süpürge", "robot supurge", "robot vacuum"],"Elektrikli Süpürge": ["elektrikli süpürge", "elektrikli supurge", "süpürge", "supurge"],"Buharlı Ütü": ["buharlı ütü", "buharli utu", "ütü", "utu"],"Buhar Kazanlı Ütü": ["buhar kazanlı", "buhar kazanli"],"Airfryer": ["airfryer", "fritöz", "fritoz"],"Espresso Kahve Makinesi": ["espresso"],"Filtre Kahve Makinesi": ["filtre kahve"],"Türk Kahvesi Makinesi": ["türk kahvesi", "turk kahvesi"],"Kahve Makinesi": ["kahve makinesi", "kahve"],"Çay Makinesi": ["çay makinesi", "cay makinesi"],"Kettle": ["kettle", "su ısıtıcı", "su isitici"],"Hava Temizleyici": ["hava temizleyici", "hava temizleme"],"Klima": ["klima"],"Akıllı Kedi Tuvaleti": ["kedi tuvaleti", "akıllı kedi", "akilli kedi"],}
...
def chatbot_pc_kurulum_tipi_bul(mesaj_lower):if any(k in mesaj_lower for k in ["tam kurulum", "monitör", "monitor", "klavye", "mouse", "fare", "hdd"]):return "Tam Kurulum"return "Temel Sistem"

def chatbot_urun_istegi_var_mi(mesaj_lower):urun_kelimeleri = ["öner", "oner", "al", "alayım", "alayim", "ne al", "bütçe", "butce", "tl", "₺","telefon", "bilgisayar", "laptop", "tablet", "kulaklık", "kulaklik", "saat","toplama", "süpürge", "supurge", "kahve", "airfryer", "ütü", "utu"]return any(kelime in mesaj_lower for kelime in urun_kelimeleri)

def chatbot_resmi_kontrol(mesaj_lower):ton_kelimeleri = ["saygılı ol", "saygili ol", "resmi konuş", "resmi konus", "kanka deme","samimi konuşma", "samimi konusma", "düzgün konuş", "duzgun konus"]return any(kelime in mesaj_lower for kelime in ton_kelimeleri)

def chatbot_kriter_mesaji(kategori, min_butce, max_butce, ram=0, kullanim=""):satirlar = ["Algılanan kriterler:",f"- Kategori: {kategori if kategori else 'Belirtilmedi'}",f"- Bütçe aralığı: {fiyat_formatla(min_butce)} - {fiyat_formatla(max_butce)}",]if ram and int(ram) > 0:satirlar.append(f"- Minimum RAM: {ram} GB")if kullanim:satirlar.append(f"- Kullanım amacı: {kullanim}")return "\n".join(satirlar)

def chatbot_sonuc_mesaji(sonuc, kategori, min_butce, max_butce, ram=0, kullanim=""):kriter = chatbot_kriter_mesaji(kategori, min_butce, max_butce, ram, kullanim)if sonuc is None:adet = 0elif isinstance(sonuc, list):adet = len(sonuc)elif hasattr(sonuc, "empty"):adet = 0 if sonuc.empty else len(sonuc)else:adet = 0if adet == 0:return ("Belirttiğiniz kriterlere uygun ürün bulunamadı.\n\n"f"{kriter}\n\n""Öneri: Bütçe aralığını artırabilir, kategori tercihini netleştirebilir veya filtreleri azaltabilirsiniz.")return (f"Belirttiğiniz kriterlere göre {adet} uygun sonuç bulundu. En uygun seçenekler aşağıda listelenmiştir.\n\n"f"{kriter}")

col_title, col_chat = st.columns([5, 1])

with col_title:st.title("🤖 Akıllı Teknoloji Ürünleri Öneri Sistemi")

with col_chat:with st.popover("💬 Chatbot"):st.subheader("Ürün Asistanı")
...
st.write("Bu sistem; hazır teknoloji ürünleri, toplama bilgisayar parçaları ve elektronik ev eşyaları için ""bütçe ve kullanım amacına göre öneri sunar.")

st.sidebar.write(f"👤 Kullanıcı: {st.session_state.user['username']}")

if st.sidebar.button("Çıkış Yap", key="logout_button"):st.session_state.logged_in = Falsest.session_state.user = Nonest.rerun()

st.sidebar.markdown("---")

with st.sidebar.expander("⭐ Favorilerim", expanded=False):favoriler = favorileri_getir(st.session_state.user["id"])
...
with st.sidebar.expander("💾 Kaydedilen Sistemlerim", expanded=False):sistemler = kayitli_sistemleri_getir(st.session_state.user["id"])
...
st.sidebar.markdown("---")st.sidebar.header("Kullanıcı Gereksinimleri")

kategori = st.sidebar.selectbox("Ürün Kategorisi",["Telefon","Bilgisayar","Tablet","Kulaklık","Akıllı Saat / Bileklik","Toplama Bilgisayar","Elektronik Ev Eşyaları","Ürün Karşılaştırma"])

st.sidebar.markdown("### Bütçe Aralığı (TL)")

butce_col1, butce_col2 = st.sidebar.columns(2)

with butce_col1:min_butce = st.number_input("Minimum",min_value=0,max_value=250000,value=int(st.session_state.get("min_butce", 10000)),step=100,key="min_butce_input")

with butce_col2:max_butce = st.number_input("Maximum",min_value=0,max_value=250000,value=int(st.session_state.get("max_butce", 30000)),step=100,key="max_butce_input")

if min_butce > max_butce:st.sidebar.warning("Minimum bütçe maximum bütçeden büyük olamaz. Değerler otomatik düzeltildi.")min_butce, max_butce = max_butce, min_butce

st.session_state.min_butce = int(min_butce)st.session_state.max_butce = int(max_butce)

siralama_secenekleri = ["Akıllı Sıralama","Popülerlik","Teknik Puan","En Düşük Fiyat","En Yüksek Fiyat","Yorum Sayısı"]

if kategori in ["Bilgisayar", "Toplama Bilgisayar"]:siralama_secenekleri.append("Bellek / RAM")

siralama = st.sidebar.selectbox("Listeyi Sırala",siralama_secenekleri)

kullanim_secimleri = {"Telefon": ["", "Günlük Kullanım", "Oyun", "Kamera / Fotoğraf", "Video İçerik Üretimi", "Sosyal Medya", "Uzun Pil", "İş"],"Bilgisayar": ["", "Oyun", "Yazılım", "Ofis", "Okul", "Tasarım", "Video Edit", "Günlük Kullanım"],"Tablet": ["", "Okul", "Not Alma", "Çizim", "Film / Dizi", "Günlük Kullanım", "İş"],"Kulaklık": ["", "Müzik", "Oyun", "Spor", "Gürültü Engelleme", "Toplantı", "Günlük Kullanım"],"Akıllı Saat / Bileklik": ["", "Spor", "Sağlık Takibi", "Günlük Kullanım", "Bildirim", "Uzun Pil"],"Toplama Bilgisayar": ["", "Oyun", "Yazılım", "Tasarım", "Video Edit", "Ofis"],"Elektronik Ev Eşyaları": ["", "Ev Temizliği", "Pratik Yemek", "Akıllı Ev", "Alerji", "Hava Kalitesi", "Güvenlik", "Günlük Kullanım"],"Ürün Karşılaştırma": [""]}

kullanim = st.sidebar.selectbox("Kullanım Amacı",kullanim_secimleri.get(kategori, [""]))

min_ram = 0

hazir_marka = "Farketmez"hazir_min_depolama = "Farketmez"hazir_min_kamera = "Farketmez"hazir_min_batarya = "Farketmez"hazir_5g = "Farketmez"hazir_nfc = "Farketmez"hazir_su = "Farketmez"hazir_gpu = "Farketmez"hazir_isletim = "Farketmez"hazir_kulaklik_tipi = "Farketmez"hazir_baglanti = "Farketmez"hazir_gurultu = "Farketmez"hazir_mikrofon = "Farketmez"hazir_min_pil = "Farketmez"hazir_gps = "Farketmez"hazir_min_pil_gun = "Farketmez"hazir_cpu_marka = "Farketmez"hazir_cpu_seviye = "Farketmez"hazir_gpu_seviye = "Farketmez"hazir_panel = "Farketmez"hazir_min_hz = "Farketmez"

ev_ana_kategori = Noneev_alt_kategori = Noneev_min_puan = 0.0ev_enerji = "Farketmez"ev_site = "Farketmez"ev_min_watt = "Farketmez"ev_min_emis = "Farketmez"ev_min_hazne = "Farketmez"ev_wifi = "Farketmez"ev_rgb = "Farketmez"ev_garanti = "Farketmez"ev_marka = "Farketmez"

if kategori == "Ürün Karşılaştırma":st.sidebar.markdown("### ⚖️ Ürün Karşılaştırma")
...
if kategori == "Toplama Bilgisayar":st.sidebar.markdown("### 🧩 Toplama Bilgisayar Modu")
...
elif kategori == "Elektronik Ev Eşyaları":st.sidebar.markdown("### 🏠 Elektronik Ev Eşyaları")
...
elif kategori in ["Telefon", "Bilgisayar", "Tablet", "Kulaklık", "Akıllı Saat / Bileklik"]:st.sidebar.markdown("### 🔎 Kategoriye Özel Filtreler")
...
if kategori not in ["Toplama Bilgisayar", "Ürün Karşılaştırma"]:if st.sidebar.button("Öneri Getir", key="oneri_getir_button"):st.session_state.pc_build = Nonest.session_state.sonuc = None
...
pc_build_ekrani_acik = (st.session_state.aktif_mod == "pc_build"or (kategori == "Toplama Bilgisayar"and "pc_mod" in locals()and pc_mod == "Bütçeye göre uyumlu sistem topla"))

if st.session_state.aktif_mod == "karsilastirma":st.subheader("⚖️ Ürün Karşılaştırma")
...
elif pc_build_ekrani_acik:st.subheader("🖥️ Bütçeye Göre 5 Farklı Uyumlu Toplama Bilgisayar Sistemi")
...
elif st.session_state.aktif_mod == "pc_parca":st.subheader("🧩 Seçimlere Göre Önerilen Toplama Bilgisayar Parçaları")
...
elif st.session_state.aktif_mod == "ev_esyalari":st.subheader("🏠 Elektronik Ev Eşyaları")
...
else:st.subheader("🔎 Önerilen Ürünler")
